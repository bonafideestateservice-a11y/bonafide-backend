...
import { getAdminProfile, updateAdminProfile } from "../../services/database/admin";
...
export const updateProfile = async (
  ...
  try {
    ...
    const current = await getAdminProfile(userId);
    if (!current) return next(new NotFoundError("Profile not found."));
    const profile = await updateAdminProfile(userId, data);
