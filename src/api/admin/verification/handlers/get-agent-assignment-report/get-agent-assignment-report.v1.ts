...
import { getAgentAssignmentReport } from "../../services/database/verification-report";
import { logger } from "../../../../../utils/logger";

export const getAgentAssignmentReportHandler = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    ...
    const report = await getAgentAssignmentReport(agent.id, req.params.id);
    if (!report) return next(new NotFoundError("Assignment report not found."));

    res.status(HttpStatusCode.OK).json(report);
  } catch (error) {
    logger.error(`Error getting agent assignment report: ${error}`);
    next(new ApiError(HttpStatusCode.INTERNAL_SERVER, "Internal server error."));
  }
};

export default getAgentAssignmentReportHandler;
