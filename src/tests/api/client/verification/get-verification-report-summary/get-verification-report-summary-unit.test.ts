...
jest.mock(
  "../../../../../api/client/verification/services/database/verification-report",
);
jest.mock("../../../../../utils/logger", () => ({
  logger: { warn: jest.fn(), error: jest.fn(), info: jest.fn() },
}));

const mockedCountUnviewedVerificationReports =
  countUnviewedVerificationReports as jest.Mock;
...
describe("getVerificationReportSummary handler (unit)", () => {
  ...
  it("returns the authenticated user's unviewed report count", async () => {
    ...
    expect(mockedCountUnviewedVerificationReports).toHaveBeenCalledWith(
      "user-1",
    );
  ...
  it("calls next with 500 when the database service fails", async () => {
    mockedCountUnviewedVerificationReports.mockRejectedValue(
